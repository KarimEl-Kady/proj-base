...
- Async listeners extend `App\Modules\Core\Listeners\QueuedListener` (`module:make {M} listener {Name} --event=X --queued`, or answer the interactive prompt): `ShouldQueue` with tries/backoff/queue from `config('project.events')`. The queue defaults to **null (the default queue)** on purpose — the shipped workers (`docker-compose` queue service, `composer dev`) only consume the default queue; if you set `PROJECT_EVENTS_QUEUE`, add it to the workers' `--queue=` list.
...
- Living examples: `Auth\Listeners\AssignDefaultRole` (sync — role assignment must be atomic with registration; typed on `Illuminate\Auth\Events\Registered`, assigns `PROJECT_AUTH_DEFAULT_ROLE` via `local/permission`) and `User\Events\UserCreated` + `User\Listeners\RecordUserCreation` (async — a DomainEvent fired from the User model's `created` hook so it fires for every entry point, with a queued audit-log listener). Tests: `AssignDefaultRoleTest`, `UserCreatedEventTest`, `DomainEventTest`.
...
- **Strict mode (fail closed, default on)**: with tenancy active, using a tenant-scoped model while no tenant is in Context throws `MissingTenantContextException` instead of silently querying every tenant's rows / creating unstamped rows. This bites code paths that don't go through `TenantMiddleware` — artisan commands, seeders, scheduled tasks, tinker (queued listeners are fine: Context, including `tenant_id`, is captured at dispatch and restored in the job). Such code must choose explicitly: `with_tenant($tenantId, fn () => ...)` to act as one tenant, or `without_tenant_scope(fn () => ...)` for deliberate cross-tenant work. An explicitly pre-set tenant column on create always wins. `PROJECT_TENANCY_STRICT=false` restores the legacy fail-open behavior. Shared resolution for CLI code lives in `Core\Support\Tenancy` (`defaultTenantId()`, `lookupTenantId()`); `user:make-admin` is tenancy-aware (single mode: runs under the default tenant automatically; multi mode: requires `--tenant={slug|subdomain}`), as is `db:seed`.
